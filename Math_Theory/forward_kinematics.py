...
class ColinearTripteronSolution:
    def __init__(self, arm_angle=30.0, alpha_tower_rotation=0.0, beta_tower_rotation=120.0, gamma_tower_rotation=240.0):
    ...
    def cartesian_to_actuator(self, cartesian_mm):
        ...
        alpha_stepper = a_x*cartesian_mm[0] - a_y*cartesian_mm[1] + cartesian_mm[2]
        beta_stepper = b_x*cartesian_mm[0] - b_y*cartesian_mm[1] + cartesian_mm[2]
        gamma_stepper = g_x*cartesian_mm[0] - g_y*cartesian_mm[1] + cartesian_mm[2]
    ...
    def actuator_to_cartesian(self, actuator_mm):
        ...
        x = (actuator_mm[0]*(g_y-b_y) + actuator_mm[1]*(a_y-g_y) + actuator_mm[2]*(b_y-a_y)) / self.d
        y = (actuator_mm[0]*(g_x-b_x) + actuator_mm[1]*(a_x-g_x) + actuator_mm[2]*(b_x-a_x)) / self.d
        z = (actuator_mm[0]*(b_y*g_x-b_x*g_y) + actuator_mm[1]*(a_x*g_y-a_y*g_x) + actuator_mm[2]*(a_y*b_x-a_x*b_y)) / self.d

        return x, y, z

# Example usage:
if __name__ == "__main__":
    kinematics = ColinearTripteronSolution()
